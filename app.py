...
#     <style>
#     .chat-container {
#         max-height: 90vh;  /* Maximum height for chat container */
...
# Set up Groq API client
key = os.getenv("GROQ_API")  # Fetch the API key from environment variables
...
# Custom CSS for chat interface and layout
st.markdown("""
    <style>
    .chat-container {
        max-height: 70vh;
...
# Show typing indicator and handle the chat
...
with st.container():
    ...
    for chat in st.session_state.history:
        st.markdown(f"<div class='clearfix'><div class='user-message'>{chat['user']}</div></div>", unsafe_allow_html=True)
        st.markdown(f"<div class='clearfix'><div class='bot-message'>{chat['bot']}</div></div>", unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)

# Add fixed input and button at the bottom
st.markdown("""
    <style>
    .stTextInput {
        position: fixed;
